...
class SCAI_Plugin {
    ...
    public function ajax_send_message() {
        ...
        // Rate limit: dual-keyed on both the hashed IP and the browser token.
        // IP key stops cost abuse even when localStorage is cleared or incognito
        // is used (resetting the token doesn't reset the IP cap).
        // Token key ensures a shared IP (office NAT) doesn't punish unrelated
        // visitors — each browser still gets its own independent 30/hr budget.
        // A request is rejected only when its own key is over the limit, so
        // one heavy user on a NAT doesn't block everyone else on that IP.
        $ip_raw   = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : 'unknown';
        $ip_salt  = function_exists( 'wp_salt' ) ? wp_salt( 'auth' ) : 'scai';
        $ip_hash  = substr( hash( 'sha256', $ip_raw . '|' . $ip_salt ), 0, 32 );
