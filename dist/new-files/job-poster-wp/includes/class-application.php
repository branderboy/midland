...
class DPJP_Application {
    ...
    public static function handle_submission(): void {
        ...
        // Rate limit: max 3 submissions per IP per hour to prevent spam.
        $ip_raw  = isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : 'unknown';
        $ip_hash = substr( hash( 'sha256', $ip_raw . '|' . wp_salt( 'auth' ) ), 0, 32 );
        $rate_key = 'dpjp_apply_rate_' . $ip_hash;
        $rate_count = (int) get_transient( $rate_key );
        if ( $rate_count >= 3 ) {
            wp_die( 'Too many applications submitted recently. Please try again in an hour.' );
        }
        ...
        // Server-side resume requirement — HTML `required` is bypassable.
        if ( $s['require_resume'] === '1' && empty( $_FILES['applicant_resume']['name'] ) ) {
            wp_die( 'A resume is required. Please go back and attach your resume.' );
        }

        // Server-side file size enforcement (5 MB limit shown in the UI).
        $max_bytes = 5 * 1024 * 1024;
        if ( ! empty( $_FILES['applicant_resume']['name'] ) && (int) $_FILES['applicant_resume']['size'] > $max_bytes ) {
            wp_die( 'Resume must be 5 MB or smaller. Please go back and attach a smaller file.' );
        }
        if ( ! empty( $_FILES['applicant_cover']['name'] ) && (int) $_FILES['applicant_cover']['size'] > $max_bytes ) {
            wp_die( 'Cover letter must be 5 MB or smaller. Please go back and attach a smaller file.' );
        }
